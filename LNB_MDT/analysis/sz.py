...
class SZ(AnalysisBase):
    ...
    def _calculate_sz_for_frame(k, head_positions, tail_residue_pos_map, 
        ...
        for sp in residues_list:
            ...
            if chain_type == 'both':
                sz_sn1_list, sz_sn2_list = None, None
                ...
                # 只有当至少有一个链有数据时才计算
                if sz_sn1_list is not None and sz_sn2_list is not None:
                    # 两者都有数据，计算平均值
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore", category=RuntimeWarning)
                        sz_sp = np.nanmean([sz_sn1_list, sz_sn2_list], axis=0)
                elif sz_sn1_list is not None:
                    # 只有 sn1 有数据
                    sz_sp = sz_sn1_list
                elif sz_sn2_list is not None:
                    # 只有 sn2 有数据
                    sz_sp = sz_sn2_list
                # 如果两者都没有数据，sz_sp 保持为初始的 np.full(num_res_sp, np.nan)
            else:
...
def main():
    ...
    if args.test:
        ...
        args.output_csv = os.path.join(test_dir, 'sz.csv')
    ...
    sz_analysis._verbose = args.verbose
    sz_analysis.run(
