...
# SET AS GLOBAL WIDGETS (module level)
# ///////////////////////////////////////////////////////////////
widgets = None

# Import PySide6 at module level (required for MainWindow class definition)
# This is acceptable because GUI functionality requires PySide6
try:
    from PySide6.QtWidgets import QApplication, QMainWindow, QTableWidget, QTableWidgetItem, QPushButton, QTextEdit
    from PySide6.QtGui import QIcon, QDragEnterEvent, QDropEvent
    from PySide6.QtCore import QObject
except ImportError:
    # If PySide6 is not installed, define placeholder classes
    # This allows the module to be imported even without PySide6
    QMainWindow = object
    QApplication = None
    QTableWidget = None
...
def test_vmd_path(vmd_path):
    ...
    if platform.system() != 'Windows':
        if not os.access(vmd_path, os.X_OK):
            return False
    
    # 尝试执行VMD的版本命令（不启动GUI）
    # 使用 -version 或 -h 参数来测试
    import subprocess
    try:
        # 尝试运行 vmd -version 或 vmd -h，设置超时避免长时间等待
        if platform.system() == 'Windows':
            # Windows下可能需要不同的参数
        ...
        # 如果命令执行成功（返回码为0或非0都可以，只要能执行）
        return True
    except (subprocess.TimeoutExpired, FileNotFoundError, OSError):
        # 如果执行失败，至少文件存在，可能路径正确但需要GUI环境
        # 这种情况下，我们仍然认为路径可能是有效的
        return os.path.exists(vmd_path)

def save_vmd_path(vmd_path):
    """保存VMD路径到配置文件"""
    config_file = get_resource_path("config.ini")
    config = load_config()
    
    # 确保VMD节存在
    if not config.has_section('VMD'):
        config.add_section('VMD')
    
    # 设置路径
    config.set('VMD', 'vmd_path', vmd_path)
    
    # 保存配置文件
    with open(config_file, 'w', encoding='utf-8') as f:
        config.write(f)
    
    print(f"✓ VMD路径已保存到配置文件: {vmd_path}")
...
def vmd_main():
    ...
    # 移除 "VMD" 参数，只保留后续参数
    if len(sys.argv) > 1 and sys.argv[1].upper() == "VMD":
        sys.argv.pop(1)
    
    parser = argparse.ArgumentParser(
        description='Configure VMD path for LNB-MDT',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  LNB-MDT VMD                    # 检查当前VMD路径
  LNB-MDT VMD --path /path/to/vmd  # 设置VMD路径
  LNB-MDT VMD -p /Applications/VMD.app/Contents/MacOS/VMD  # macOS示例
        """
    )
    ...
    # 获取当前配置的路径
    config = load_config()
    current_path = config.get('VMD', 'vmd_path', fallback=None)
    ...
    if args.path:
        ...
        if not os.path.exists(new_path):
            print(f"✗ 错误: 路径不存在: {new_path}")
            print("\n请检查路径是否正确，常见路径示例:")
            if platform.system() == 'Windows':
                print("  Windows: C:/Program Files/VMD/vmd.exe")
            elif platform.system() == 'Darwin':
                print("  macOS: /Applications/VMD.app/Contents/MacOS/VMD")
            else:
                print("  Linux: /usr/local/bin/vmd")
            sys.exit(1)
        
        # 测试路径是否可用
        print(f"正在测试VMD路径: {new_path}")
        if test_vmd_path(new_path):
            save_vmd_path(new_path)
            print("✓ VMD路径配置成功！")
        else:
            print(f"⚠ 警告: 无法验证VMD路径是否可用: {new_path}")
            print("路径存在，但可能无法执行。是否仍要保存？(y/n): ", end='')
            try:
        ...
        print(f"当前VMD路径: {current_path}")
        
        # 测试路径是否可用
        print("正在检测VMD路径...")
        if test_vmd_path(current_path):
            print("✓ VMD路径有效，可以正常使用")
        else:
            print("✗ VMD路径无效或无法访问")
...
class MainWindow(QMainWindow):
        def __init__(self):
            ...
            from LNB_MDT.modules.vmd_control import VMDCommands, VMDTcp
            from LNB_MDT.modules import (
                Ui_MainWindow, Settings, UIFunctions, AppFunctions, 
                BtnGetPath, BtnGeneClick, lipidsSelect, NextClick, FigurePage
            )
            from LNB_MDT.modules.file_detection import FileTypeDetector, DynamicTabManager
            from LNB_MDT.modules.theme_manager import ThemeManager, add_theme_button_to_top_menu
            
            # Store imports as instance variables for use in methods
            self.VMDCommands = VMDCommands
            self.VMDTcp = VMDTcp
            self.Ui_MainWindow = Ui_MainWindow
            self.Settings = Settings
            self.UIFunctions = UIFunctions
            self.AppFunctions = AppFunctions
            self.BtnGetPath = BtnGetPath
            self.BtnGeneClick = BtnGeneClick
            self.lipidsSelect = lipidsSelect
            self.NextClick = NextClick
            self.FigurePage = FigurePage
            self.FileTypeDetector = FileTypeDetector
            self.DynamicTabManager = DynamicTabManager
            self.ThemeManager = ThemeManager
            self.add_theme_button_to_top_menu = add_theme_button_to_top_menu
            ...
            widgets = self.ui
            # 初始化 VMD 相关
            self.rctl_path = get_resource_path("remote_ctl.tcl")
            ...
            # ///////////////////////////////////////////////////////////////
            self.Settings.ENABLE_CUSTOM_TITLE_BAR = True
            
            # TOGGLE MENU
            # ///////////////////////////////////////////////////////////////
            widgets.toggleButton.clicked.connect(lambda: self.UIFunctions.toggleMenu(self, True))
            
            # SET UI DEFINITIONS
            self.UIFunctions.uiDefinitions(self)
            ...
            def openCloseRightBox():
                self.UIFunctions.toggleRightBox(self, True)
            
            def openCloseGeneBox():
                self.UIFunctions.toggleGeneRightBox(self, True)
            
            def openCloseFigureColorBox():
                if self.ui.figure_extra == 1:
                    self.UIFunctions.toggleFigureColorBox(self, True)
                elif self.ui.figure_extra == 0:
                    self.UIFunctions.toggleFigureColorBox(self, False)
            
            def openCloseFigureShapeBox():
                self.UIFunctions.toggleFigureShapeBox(self, True)
            
            def openCloseFigureExtra(index):
                if index == 0 or index == 1:
                    self.UIFunctions.toggleFigureShapeBox(self, False)
                elif index == 2:
                    self.UIFunctions.toggleFigureColorBox(self, False)
            
            self.ui.btn_language.clicked.connect(lambda: self.AppFunctions.btnLanguageClick(self.ui))
            # //////////////////////////Generation/////////////////////////////////////
            # btnGeneration Click
            # self.ui.btn_gene_run
            self.ui.btn_gene_path.clicked.connect(lambda: self.BtnGetPath.run(self.ui.edit_gene_path, 'gene_gro'))
            self.ui.btn_gene_run.clicked.connect(lambda: self.BtnGeneClick(self.ui))
    
            self.ui.btn_gene_lipid.clicked.connect(openCloseGeneBox)
            self.ui.btn_gene_lipid.clicked.connect(lambda: self.lipidsSelect(self.ui))
            # //////////////////////////Analysis/////////////////////////////////////
            # btnTools CLICK
            # ///////////////////////////////////////////////////////////////
            # 按钮已在ui_main.py中使用make_btn创建，这里只需要绑定事件
            self.ui.btnSructure.clicked.connect(lambda: self.BtnGetPath.run(self.ui.editStructure, 'analysis_gro'))
            self.ui.btnTrajectory.clicked.connect(lambda: self.BtnGetPath.run(self.ui.editTrajectory, 'analysis_xtc'))
            self.ui.btnResult.clicked.connect(lambda: self.BtnGetPath.run(self.ui.editResult, 'analysis_result'))
    
            self.ui.btnNext.clicked.connect(openCloseRightBox)
            self.ui.btnNext.clicked.connect(lambda: self.NextClick(self.ui))
            ...
            self.ui.btn_figure_run.clicked.connect(lambda: self.FigurePage.figureBtnMakeFigure(self.ui))
            self.ui.figure_btn_path.clicked.connect(lambda: self.BtnGetPath.run(self.ui.figure_edit_path, 'figure_xlsx'))
            ...
            widgets.btn_home.setStyleSheet(self.UIFunctions.selectMenu(widgets.btn_home.styleSheet()))
        ...
        def loadCSV(self, csv_path):
            ...
            try:
                ...
                from LNB_MDT.modules.vmd_control import read_excel_vmd
        ...
        def pushStartVMD(self):
            try:
                self.vmd = self.VMDTcp(self.rctl_path, self.vmd_path)
                ...
                if response == -1:
                    ...
                    if self.gro_file and self.xtc_file:
                        ...
                        if gro_exists and xtc_exists:
                            ...
                            self.vmd.send_command(self.VMDCommands.setupInitialDisplay())
        ...
        def onSelectionChanged(self):
            ...
            # 1. 跳转到指定帧
            self.vmd.send_command(self.VMDCommands.gotoFrame(frame))
            ...
            if not self.beta_coloring_enabled and hasattr(self, 'csv_min_value') and hasattr(self, 'csv_max_value'):
                if self.csv_min_value is not None and self.csv_max_value is not None:
                    self.vmd.send_command(self.VMDCommands.setupBetaColoring(self.csv_min_value, self.csv_max_value))
                    self.beta_coloring_enabled = True
            
            # 3. 设置所有resid的beta值
            if resid_value_dict:
                self.vmd.send_command(self.VMDCommands.setBetaValues(resid_value_dict))
            
            # 4. 高亮显示选中的resid
            self.vmd.send_command(self.VMDCommands.highlightResid(resids))
        ...
        def buttonLeftClick(self):
            ...
            if btnName == "btn_home":
                widgets.stackedWidget.setCurrentWidget(widgets.page_home)
                # 用来改变侧边按钮点击后的显示
                self.UIFunctions.resetStyle(self, btnName)
                btn.setStyleSheet(self.UIFunctions.selectMenu(btn.styleSheet()))
            ...
            if btnName == "btn_figure":
                ...
                self.UIFunctions.resetStyle(self, btnName)  # RESET ANOTHERS BUTTONS SELECTED
                btn.setStyleSheet(self.UIFunctions.selectMenu(btn.styleSheet()))  # SELECT MENU
        ...
        def resizeEvent(self, event):
            # Update Size Grips
            self.UIFunctions.resize_grips(self)
        ...
        def setup_file_detection(self):
            """设置文件检测功能"""
            # 创建动态tab管理器，传入UI实例
            self.tab_manager = self.DynamicTabManager(self.ui)
        ...
        def setup_theme_switch(self):
            ...
            try:
                # 将主题切换按钮添加到顶部菜单
                self.theme_manager = self.add_theme_button_to_top_menu(self.ui, self)
            except Exception as e:
                # 如果添加失败，至少创建主题管理器
                self.theme_manager = self.ThemeManager(self)
...
"""
        ...
        def auto_detect_file_type(self, file_path):
            ...
            try:
                # 检测文件类型
                file_type = self.FileTypeDetector.detect_file_type(file_path)
        ...
        def _ensure_figure_info_for_file(self, file_path):
            ...
            try:
                ...
                from LNB_MDT.modules.Fuctions_Figure import create_parameter_reader
...
def run_gui():
    ...
    # Import all necessary modules (lazy import to avoid circular dependencies)
    from LNB_MDT.modules.vmd_control import VMDCommands, VMDTcp
    from LNB_MDT.modules import (
        Ui_MainWindow, Settings, UIFunctions, AppFunctions, 
        BtnGetPath, BtnGeneClick, lipidsSelect, NextClick, FigurePage
    )
    from LNB_MDT.widgets import CustomGrip
    from LNB_MDT.figure import read_excel, LipidsFigure, BubbleFigure, DensityFigure
    from LNB_MDT.analysis import Area, DensityTime, DensityRadius
    from LNB_MDT.modules.file_detection import FileTypeDetector, DynamicTabManager
    from LNB_MDT.modules.theme_manager import ThemeManager, add_theme_button_to_top_menu
...
def main():
    """Main CLI dispatcher"""
    if len(sys.argv) < 2 or sys.argv[1].upper() == "UI":
        # Run GUI
        run_gui()
    elif sys.argv[1].upper() == "AREA":
        # Remove "AREA" from argv so area.py sees standard args
        sys.argv.pop(1)
        from LNB_MDT.analysis.area import main as area_main
        area_main()
    elif sys.argv[1].upper() == "DENSITY":
        # Remove "DENSITY" from argv
        sys.argv.pop(1)
        from LNB_MDT.analysis.density import main as density_main
        density_main()
    elif sys.argv[1].upper() == "SZ":
        # Remove "SZ" from argv
        sys.argv.pop(1)
        from LNB_MDT.analysis.sz import main as sz_main
        sz_main()
    elif sys.argv[1].upper() == "HEIGHT":
        # Remove "HEIGHT" from argv
        sys.argv.pop(1)
        from LNB_MDT.analysis.height import main as height_main
        height_main()
    elif sys.argv[1].upper() == "ANISOTROPY":
        # Remove "ANISOTROPY" from argv
        sys.argv.pop(1)
        from LNB_MDT.analysis.anisotropy import main as anisotropy_main
        anisotropy_main()
    elif sys.argv[1].upper() == "CLUSTER":
        # Remove "CLUSTER" from argv
        sys.argv.pop(1)
        from LNB_MDT.analysis.cluster import main as cluster_main
        cluster_main()
    elif sys.argv[1].upper() == "GYRATION":
        # Remove "GYRATION" from argv
        sys.argv.pop(1)
        from LNB_MDT.analysis.gyration import main as gyration_main
        gyration_main()
    elif sys.argv[1].upper() == "FIGURE":
        # Remove "FIGURE" from argv
        sys.argv.pop(1)
        from LNB_MDT.figure.make_figure import main as figure_main
        figure_main()
    elif sys.argv[1].upper() == "GEN":
        # Remove "GEN" from argv
        sys.argv.pop(1)
        from LNB_MDT.preparation.lnb_generate_with_folder import main as gen_main
        gen_main()
    elif sys.argv[1].upper() == "VMD":
        # VMD path configuration command
        vmd_main()
    else:
        # Print help message
        print("LNB-MDT - Lipid Nanobubble Molecular Dynamics Toolkit")
        print("\nUsage: LNB-MDT <SUBCOMMAND> [OPTIONS]")
        print("\nAvailable subcommands:")
        print("  UI         Launch the graphical user interface (default)")
        print("  AREA       Run area analysis")
        print("  DENSITY    Run density analysis")
        print("  SZ         Run Sz order parameter analysis")
        print("  HEIGHT     Run height analysis")
        print("  ANISOTROPY Run anisotropy analysis")
        print("  CLUSTER    Run cluster analysis")
        print("  GYRATION   Run gyration analysis")
        print("  FIGURE     Generate figures from CSV files")
        print("  GEN        Generate LNB system")
        print("  VMD        Configure VMD path")
        print("\nFor help on a specific subcommand, use:")
        print("  LNB-MDT <SUBCOMMAND> --help")
