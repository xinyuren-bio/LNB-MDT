...
from LNB_MDT.figure import *
from .Tools import *
from LNB_MDT.analysis import *
...
class AnalysisBtnClick:
    ...
    def update_button_icons(self):
        ...
        try:
            ...
            # 使用 get_resource_path 获取正确的图标路径
            def get_resource_path(relative_path):
                ...
                try:
                    ...
                    # 获取 LNB_MDT 包的路径
                    import LNB_MDT
                    base_path = os.path.dirname(os.path.abspath(LNB_MDT.__file__))
                return os.path.join(base_path, relative_path)
            
            # 更新按钮图标
            if hasattr(self.ui, 'btnBack'):
                back_icon_full_path = get_resource_path(back_icon_path)
                if os.path.exists(back_icon_full_path):
                    self.ui.btnBack.setIcon(QIcon(back_icon_full_path))
                else:
                    # 如果文件不存在，尝试使用资源文件路径
                    self.ui.btnBack.setIcon(QIcon(f":/icons/{back_icon_path}"))
            
            if hasattr(self.ui, 'btnRefresh'):
                refresh_icon_full_path = get_resource_path(refresh_icon_path)
                if os.path.exists(refresh_icon_full_path):
                    self.ui.btnRefresh.setIcon(QIcon(refresh_icon_full_path))
                else:
                    # 如果文件不存在，尝试使用资源文件路径
                    self.ui.btnRefresh.setIcon(QIcon(f":/icons/{refresh_icon_path}"))
            ...
            if hasattr(self.ui, 'btnBack'):
                back_icon_full_path = get_resource_path('images/icons/houtui.png')
                if os.path.exists(back_icon_full_path):
                    self.ui.btnBack.setIcon(QIcon(back_icon_full_path))
                else:
                    self.ui.btnBack.setIcon(QIcon(':/icons/images/icons/houtui.png'))
            if hasattr(self.ui, 'btnRefresh'):
                refresh_icon_full_path = get_resource_path('images/icons/shuaxin.png')
                if os.path.exists(refresh_icon_full_path):
                    self.ui.btnRefresh.setIcon(QIcon(refresh_icon_full_path))
                else:
                    self.ui.btnRefresh.setIcon(QIcon(':/icons/images/icons/shuaxin.png'))
